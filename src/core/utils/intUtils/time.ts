...
const convertTime =
	<A, B, C>(fn1: TimePipe<A, B>, fn2: TimePipe<B, C>) =>
	(input: A): C =>
		fn2(fn1(input));
...
export const CurrentWorkingMilliseconds: CurrentWorkingTime = () =>
	CurrentTime("m") - START_TIME;
export const CurrentWorkingSeconds: CurrentWorkingTime = () =>
	ToSeconds(CurrentTime("m") - START_TIME);
export const CurrentWorkingMinutes: CurrentWorkingTime = () =>
	ToMinutes(CurrentTime("m") - START_TIME);
export const CurrentWorkingHours: CurrentWorkingTime = () =>
	ToHours(CurrentTime("m") - START_TIME);

//* milliseconds -> seconds
type ToSeconds = (milliseconds: number) => number;
export const ToSeconds: ToSeconds = (milliseconds) =>
	milliseconds > MILLISECONDS_IN_SECOND && milliseconds > 0
		? Math.trunc(milliseconds / MILLISECONDS_IN_SECOND)
		: 0;

//* milliseconds -> minutes
type ToMinutes = (miliseconds: number) => number;
export const ToMinutes: ToMinutes = (milliseconds) =>
	Math.trunc(ToSeconds(milliseconds) / SECONDS_IN_MINUTE);

//* milliseconds -> hours
type ToHours = (milliseconds: number) => number;
export const ToHours: ToHours = (milliseconds) =>
	Math.trunc(ToMinutes(milliseconds) / MINUTES_IN_HOUR);

//* seconds -> milliseconds
type FromSeconds = (seconds: number) => number;
export const FromSeconds: FromSeconds = (seconds) =>
	seconds * MILLISECONDS_IN_SECOND;

//* hours -> milliseconds
type FromHours = (hours: number) => number;
export const FromHours: FromHours = (hours) =>
	FromMinutes(hours * MINUTES_IN_HOUR);

//* minutes -> milliseconds
type FromMinutes = (minutes: number) => number;
export const FromMinutes: FromMinutes = (minutes) =>
	FromSeconds(minutes * SECONDS_IN_MINUTE);
...
type ToTimeStamp = (milliseconds: number) => TimeStamp;
export const ToTimeStamp: ToTimeStamp = (milliseconds: number): TimeStamp => {
